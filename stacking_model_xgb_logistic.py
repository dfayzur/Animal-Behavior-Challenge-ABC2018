...
def stacking_model_xgb_logistic(task='together'):
    nfold = 5
    #task='together'
    ...
    #oof_preds_folds = pd.DataFrame(oof_preds_folds,columns=['xgb_logistic_seed_' + str(seeds[l]) for l in range(len(seeds))])
    #sub_preds_folds = pd.DataFrame(sub_preds_folds,columns=['xgb_logistic_seed_' + str(seeds[l]) for l in range(len(seeds))])
    oof_preds_folds_vote = pd.DataFrame(oof_preds_folds_vote,columns=['xgb_logistic_seed_' + str(seeds[l]) for l in range(len(seeds))])
    sub_preds_folds_vote = pd.DataFrame(sub_preds_folds_vote,columns=['xgb_logistic_seed_' + str(seeds[l]) for l in range(len(seeds))])

    #oof_preds_folds.to_csv("./output/" + task + "_train_stack/xgb_binary.csv", index=False)
    #sub_preds_folds.to_csv("./output/" + task + "_test_stack/xgb_binary.csv", index=False)
    oof_preds_folds_vote.to_csv("./output/" + task + "_train_stack_vote/xgb_binary.csv", index=False)
    sub_preds_folds_vote.to_csv("./output/" + task + "_test_stack_vote/xgb_binary.csv", index=False)
    feature_importance_df_folds=feature_importance_df_folds.sort_values('importance', ascending=False)
    feature_importance_df_folds.to_csv("./output/" + task + "_feature/xgb_binary.csv", index=False)
