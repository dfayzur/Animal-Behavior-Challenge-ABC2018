...
def stacking_model_sk_gbt(task='together'):
    nfold = 5
    #task='together'
    ...
    #oof_preds_folds = pd.DataFrame(oof_preds_folds,columns=['sk_gbt_seed_' + str(seeds[l]) for l in range(len(seeds))])
    #sub_preds_folds = pd.DataFrame(sub_preds_folds,columns=['sk_gbt_seed_' + str(seeds[l]) for l in range(len(seeds))])
    oof_preds_folds_vote = pd.DataFrame(oof_preds_folds_vote,columns=['sk_gbt_seed_' + str(seeds[l]) for l in range(len(seeds))])
    sub_preds_folds_vote = pd.DataFrame(sub_preds_folds_vote,columns=['sk_gbt_seed_' + str(seeds[l]) for l in range(len(seeds))])

    #oof_preds_folds.to_csv("./output/" + task + "_train_stack/sk_gbt.csv", index=False)
    #sub_preds_folds.to_csv("./output/" + task + "_test_stack/sk_gbt.csv", index=False)
    oof_preds_folds_vote.to_csv("./output/" + task + "_train_stack_vote/sk_gbt.csv", index=False)
    sub_preds_folds_vote.to_csv("./output/" + task + "_test_stack_vote/sk_gbt.csv", index=False)
