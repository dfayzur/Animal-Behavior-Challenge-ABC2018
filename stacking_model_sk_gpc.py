...
def stacking_model_sk_gpc(task='together'):
    nfold = 5
    #task='together'
    ...
    for seed_id in range(len(seeds)):
        ...
        clf = GaussianProcessClassifier(kernel = kernel, max_iter_predict=param['max_iter_predict'],
                                         n_restarts_optimizer=param['n_restarts_optimizer'],
                  n_jobs=-1,
                  random_state=seeds[seed_id])
    ...
    #oof_preds_folds = pd.DataFrame(oof_preds_folds,columns=['sk_gpc_seed_' + str(seeds[l]) for l in range(len(seeds))])
    #sub_preds_folds = pd.DataFrame(sub_preds_folds,columns=['sk_gpc_seed_' + str(seeds[l]) for l in range(len(seeds))])
    oof_preds_folds_vote = pd.DataFrame(oof_preds_folds_vote,columns=['sk_gpc_seed_' + str(seeds[l]) for l in range(len(seeds))])
    sub_preds_folds_vote = pd.DataFrame(sub_preds_folds_vote,columns=['sk_gpc_seed_' + str(seeds[l]) for l in range(len(seeds))])

    #oof_preds_folds.to_csv("./output/" + task + "_train_stack/sk_gpc.csv", index=False)
    #sub_preds_folds.to_csv("./output/" + task + "_test_stack/sk_gpc.csv", index=False)
    oof_preds_folds_vote.to_csv("./output/" + task + "_train_stack_vote/sk_gpc.csv", index=False)
    sub_preds_folds_vote.to_csv("./output/" + task + "_test_stack_vote/sk_gpc.csv", index=False)
