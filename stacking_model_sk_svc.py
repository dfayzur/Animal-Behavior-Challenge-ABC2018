...
def stacking_model_sk_svc(task='together'):
    ...
    oof_preds_folds_vote = pd.DataFrame(oof_preds_folds_vote,columns=['sk_svc_seed_' + str(seeds[l]) for l in range(len(seeds))])
    sub_preds_folds_vote = pd.DataFrame(sub_preds_folds_vote,columns=['sk_svc_seed_' + str(seeds[l]) for l in range(len(seeds))])
    ...
    oof_preds_folds_vote.to_csv("./output/" + task + "_train_stack_vote/sk_svc.csv", index=False)
    sub_preds_folds_vote.to_csv("./output/" + task + "_test_stack_vote/sk_svc.csv", index=False)
